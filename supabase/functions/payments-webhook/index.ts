...
async function handleTransactionCompleted(data: any, env: PaddleEnv) {
  // One-time purchase support (e.g. audit_defense_memo). Subscription items already handled.
  const userId = data.customData?.userId;
  if (!userId || data.subscriptionId) return;
  const item = data.items?.[0];
  const priceId = item?.price?.importMeta?.externalId;
  if (priceId === 'audit_defense_memo' || priceId === 'audit_defense_yearly') {
    // Flag profile add-on for audit defense.
    await getSupabase().from('profiles')
      .update({ addon_audit_defense: true })
      .eq('user_id', userId);
  }
}
