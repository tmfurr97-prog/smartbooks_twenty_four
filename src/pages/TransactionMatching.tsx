...
export default function TransactionMatchingPage() {
  ...
  const runMatching = () => {
    ...
    setTimeout(() => {
      ...
      setUnmatchedTransactions(results.unmatchedTransactions);
      setUnmatchedReceipts(results.unmatchedReceipts);
      setIsMatching(false);
    }, 1000);
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-heading text-2xl md:text-3xl font-bold text-foreground">Transaction Matching</h1>
          <p className="text-muted-foreground mt-1">Automatically link bank transactions with receipts</p>
        </div>
        <Button onClick={runMatching} disabled={isMatching}>
          <RefreshCw className={`h-4 w-4 mr-2 ${isMatching ? 'animate-spin' : ''}`} />Re-match All
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-muted-foreground">Matched</p><p className="text-3xl font-bold text-green-600">{matched.length}</p></div>
            <div className="h-12 w-12 rounded-full bg-green-500/10 flex items-center justify-center"><FileText className="h-6 w-6 text-green-600" /></div>
          </div>
        </Card>
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-muted-foreground">Missing Receipts</p><p className="text-3xl font-bold text-orange-600">{unmatchedTransactions.length}</p></div>
            <div className="h-12 w-12 rounded-full bg-orange-500/10 flex items-center justify-center"><CreditCard className="h-6 w-6 text-orange-600" /></div>
          </div>
        </Card>
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div><p className="text-sm text-muted-foreground">Unmatched Receipts</p><p className="text-3xl font-bold text-blue-600">{unmatchedReceipts.length}</p></div>
            <div className="h-12 w-12 rounded-full bg-blue-500/10 flex items-center justify-center"><AlertCircle className="h-6 w-6 text-blue-600" /></div>
          </div>
        </Card>
      </div>
      ...
      <Tabs defaultValue="matched">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="missing-receipts">Missing Receipts ({unmatchedTransactions.length})</TabsTrigger>
          <TabsTrigger value="unmatched-receipts">Unmatched Receipts ({unmatchedReceipts.length})</TabsTrigger>
        </TabsList>
        <TabsContent value="matched" className="space-y-4 mt-4">
          {matched.length === 0 ? (
            <Alert><AlertDescription>No matched transactions found. Upload receipts to start matching.</AlertDescription></Alert>
          ) : matched.map(pair => <MatchedPairCard key={pair.transaction.id} pair={pair} onUnmatch={handleUnmatch} />)}
        </TabsContent>
            ...
            <Card key={t.id} className="p-4">
              <div className="flex items-start justify-between">
                <div className="space-y-2">
                  <div className="flex items-center gap-2"><Badge variant="outline">{t.source}</Badge><Badge variant="destructive">Missing Receipt</Badge></div>
                ...
                <div className="text-right">
                  <p className="text-2xl font-bold text-foreground">${t.amount.toFixed(2)}</p>
        ...
        <TabsContent value="unmatched-receipts" className="space-y-4 mt-4">
          {unmatchedReceipts.map(r => (
            <Card key={r.id} className="p-4">
              <div className="flex items-start justify-between">
                <div className="space-y-2">
                  <div className="flex items-center gap-2"><Badge>{r.category}</Badge><Badge variant="secondary">No Transaction</Badge></div>
                  <p className="font-semibold text-foreground">{r.merchant}</p>
